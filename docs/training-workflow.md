...
同类产物必须遵循固定格式。完整约定、校验范围和历史兼容说明见 [产物格式约定](artifact-formats.md)。新运行的 `process/formats.json` 可下载查看文件结构、数组 dtype、Schema 和报告模板指纹。

文件目录中的重复产物默认按类型折叠，显示文件数量与总大小；展开后保留每个文件的完整路径和下载入口。BIDS 文件按相同任务、空间、后缀及扩展名归组，合并不同被试、会话和 Run；预处理与交付溯源按相同相对文件名归组，保留滤波、重参考、分段等步骤的区别。仅有一个文件的类型及汇总文件直接展示。轮询新增记录时，已展开的同类组保持展开。
