...
正式流程前的重建修订采用 `dataset-reconstruction-v5`：脉冲由整段随机抽样改为固定 0.25 秒块内的确定性随机脉冲，避免没有上界的空白间隔。块模板保持单位 RMS，最后按整个源记录统一缩放；源信号的全零短片段不再导致模板生成失败。该波形是工程探针，不模拟自然伪迹发生率。109 人条件分配、种子、整体 RMS 强度、共同视图、阈值和试次分母均沿用 v4；短于覆盖范围的窗口仍可能不适用，不能据此删除试次。新旧探针哈希不同，历史回执不升级。

新版真实物理基线重建专项已通过：109 人的 109 个分配案例、1640 个冻结探针试次全部可评估，109 次干净及109次污染回放、111.408秒。干净回放最大绝对差约1.0842e-19 V，位于原容差内；源数据、历史搜索和冻结代码哈希不变。203项最终专项回归通过（34.72秒）。原型共享图检查保留102可评估、7个全零短块失败（758.093秒），源数据、图产物与代码也均不变。详见 [新版探针复核](sources/remaining-modifications-20260912/probe-v5-review.json)。这些结果不包含新的EEGNet训练、完整候选评价或文献闭环验收。

第9次正式全集流程在冻结提交 `560f5ec` 上于第一次研究规划请求前被原750000字节输入上限拦截，未进入数值搜索，源数据和代码不变。发现1526条WFDB比较重复了相同观测信息；新上下文按9组相同结果无损分组，保留每条记录的文件/哈希/字节数、全部差异和错误。完整请求封装的预检从1125408降至370089字节，1260条一致和266条不一致可精确还原，21项相关测试通过（2.56秒）。没有提高上限或把失败请求作为完成；见 [请求预检](sources/remaining-modifications-20260912/wfdb-context-preflight.json)。后续须创建新流程并重新取得来源。
