...
新运行在 `process/formats.json` 的 `survey_reports` 中冻结版本 2、六个文件名和 HTML 模板 SHA-256。产物格式 6 使用有类型的本地观测结构 2，并增加 `local-events.tsv`；报告按范围、文件组织、信号、事件、元数据、统计与待办六组展示。记录详情使用独立滚动窗口，Esc 或“关闭详情”返回；共用通道配置只展示一次。历史运行仍可读取，不能以新格式续跑旧快照。
...
默认根据本地路径选择全部被试，明确列出扫描人数、选择人数和记录数。阅读区超过 20 行的总表和被试列表支持搜索及分页；记录详情按被试折叠。下载的 HTML 保留完整记录，长表在固定高度内滚动。共享属性和范围摘要压缩长编号列表，完整名单保存在结构化记录及明细中。
