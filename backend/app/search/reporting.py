...
def render(root: Path, state):
    ...
    for a in state["actions"]:
        actions.append(
            f"<details><summary>{a['index']} · {escape(a['action'])} · {escape(a['status'])}</summary>"
            f"<p>{escape(a['reason'])}</p><p>{escape(a.get('expected_result') or '')}</p>"
            f"<p>{escape(str(a.get('decision_branches') or ''))}</p><p>{escape(a.get('error') or '')}</p></details>"
        )
    per_subject = []
    if selected:
        for subject, row in (selected.get("receipt") or {}).get("subjects", {}).items():
            per_subject.append(
                f"<tr><td>{escape(subject)}</td><td>{score(row.get('ba'))}</td><td>{score(row.get('delta'))}</td></tr>"
            )
...
<p class="muted">{escape(state["stop_reason"] or "")} · 累计 {state["usage"]["elapsed_seconds"]:.1f} 秒 ·
...
<p>固定资料和参数范围后才开始搜索。当前源数据此前已用于开发，不能视为未见确认集。</p></main></html>"""
