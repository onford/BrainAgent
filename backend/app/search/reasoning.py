...
固定任务是左右手运动想象，离线完整记录处理后切窗。被试折、trial、输出网格和共同评价器已冻结。选择主指标为assessment.selection_score：CSP-LDA、FBCSP、TS-LR三种基本模型各自被试等权BA的等权平均，三者必须完整。训练、特征选择、内层调参只使用外层训练被试；FgMDM、逐频带EA-FBCSP及对数方差逻辑回归是独立对照，不决定胜者。macro_ba保留为CSP锚点，不能冒充三模型主指标。
