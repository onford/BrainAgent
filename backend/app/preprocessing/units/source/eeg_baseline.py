...
def _eeg_baseline_baseline(op, x, **p):
    y = _copy(x)
    _args(p, 'baseline')
    _sequence(p['baseline'], 'baseline', 2)
    for bound in p['baseline']:
        if bound is not None:
            _number(bound, 'baseline')
    if not isinstance(y, mne.BaseEpochs) or p['baseline'] is None:
        raise ValueError('需要 Epochs 和基线区间')
    y.apply_baseline(p['baseline'])
    return _out(y)

def eeg_baseline(op, x, model=None, **p):
    if op == 'baseline':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _eeg_baseline_baseline(op, x, **p)
