...
def _eeg_rest_rest(op, x, **p):
    y = _copy(x)
    _args(p, 'forward')
    if p['forward'] is None:
        raise ValueError('REST 缺 forward')
    y.set_eeg_reference(ref_channels='REST', forward=p['forward'], projection=False)
    return _out(y, unit='V', representation='potential')

def eeg_rest(op, x, model=None, **p):
    if op == 'rest':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _eeg_rest_rest(op, x, **p)
