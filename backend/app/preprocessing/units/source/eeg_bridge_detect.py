...
def _eeg_bridge_detect_bridge_detect(op, x, **p):
    y = _copy(x)
    before = y.get_data().copy()
    _args(p, 'lm_cutoff epoch_threshold l_freq h_freq epoch_duration')
    ...
    _number(p['lm_cutoff'], 'lm_cutoff', 0, strict_low=True)
    _number(p['epoch_threshold'], 'epoch_threshold', 0, 1)
    _number(p['epoch_duration'], 'epoch_duration', 0, strict_low=True)
    (pairs, distance) = mne.preprocessing.compute_bridged_electrodes(y, **p)
    return _out(y, pairs=pairs, electrical_distance=distance, distance_unit='uV^2', distance_channel_indices=mne.pick_types(y.info, eeg=True, exclude='bads').tolist(), distance_channel_names=[y.ch_names[i] for i in mne.pick_types(y.info, eeg=True, exclude='bads')])

def eeg_bridge_detect(op, x, model=None, **p):
    if op == 'bridge_detect':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _eeg_bridge_detect_bridge_detect(op, x, **p)
