...
def _drop_bad(op, x, **p):
    _args(p, 'channels reason')
    y = _copy(x)
    _channels(y, p['channels'])
    if not set(p['channels']) <= set(y.info['bads']):
        raise ValueError('只能剔除已确认坏EEG，不能用本入口选择ROI')
    if not isinstance(p['reason'], str) or not p['reason'].strip():
        raise ValueError('缺少剔除理由')
    if not set((n for (n, t) in zip(y.ch_names, y.get_channel_types()) if t == 'eeg')) - set(p['channels']) - set(y.info['bads']):
        raise ValueError('至少保留一个好EEG')
    y.drop_channels(p['channels'])
    return _out(y, removed=list(p['channels']), kept=y.ch_names, reason=p['reason'])

def eeg_bad_channel_drop(op, x, model=None, **p):
    if op == 'drop':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _drop_bad(op, x, **p)
