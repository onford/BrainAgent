...
def _eeg_csd_csd(op, x, **p):
    y = _copy(x)
    _args(p, 'sphere lambda2 stiffness n_legendre_terms')
    if not isinstance(p['sphere'], str) or p['sphere'] != 'auto':
        _vector(p['sphere'], 'sphere', 4)
    y = mne.preprocessing.compute_current_source_density(y, copy=True, **p)
    return _out(y, unit='V/m^2', representation='csd')

def eeg_csd(op, x, model=None, **p):
    if op == 'csd':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _eeg_csd_csd(op, x, **p)
