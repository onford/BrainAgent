...
def eeg_kurtosis(op,x,model=None,**p):
    p=_params(p,{'local_threshold': 10.0, 'global_threshold': 10.0})
    a,names,sf=_start(op,'kurtosis',x,model,True)
    ...
    local=stats.kurtosis(a,axis=-1,fisher=False,bias=True).T
    global_raw=stats.kurtosis(a.reshape(len(a),-1),axis=-1,fisher=False,bias=True)
