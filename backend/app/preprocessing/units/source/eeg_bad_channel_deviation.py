...
def eeg_bad_channel_deviation(op,x,model=None,**p):
    if op != 'deviation_detect':raise ValueError("invalid operation")
    p=_params(p,{'do_detrend': True, 'matlab_strict': True, 'random_state': 0, 'reject_by_annotation': None, 'deviation_threshold': 5.0})
    methods={'deviation':{k:p.pop(k) for k in ['deviation_threshold']}}
