...
_PINS={'eeglab/functions/popfunc/eeg_emptyset.m': '5359ea9eeb4dc8c85c3465cf6ef7ad8c769f839f027734b268e2b37018cb76be', 'eeglab/functions/popfunc/eeg_getica.m': '46ec2820d4841f2a4c3706bf0e1ba91aa1960a51f95800a24653dedee40b3776', 'eeglab/functions/adminfunc/eeg_checkset.m': 'fa6356b4a64ea6e16683acc4c65b257084dd1bfc995848de402244ccba00d6f0', 'eeglab/functions/sigprocfunc/kurt.m': '987072d6bdf64188c08ae3b85a2815fb357bd6bec034f7b044d33c4c8c169688', 'MARA/MARA.m': '3ed2c0778652c178ffc778f1328b2ea8a4d6d3085386dae817b46e566a6c1931', 'MARA/fv_training_MARA.mat': '636d47cefc046451bec5cebb8ba7b9c16cd5e573fc3469ee615fa18974f1291c', 'MARA/inv_matrix_icbm152.mat': '205349c4a8a6f3503234b4a4bffb69ffd470a1437463c1c9237f5419d1f6ea54'}
def eeg_mara(op,x,model=None,**p):
    if op!='mara_assess':raise NotImplementedError(op)
    y=_copy(x);p=_exact(p,'source_root reference_id',octave='octave',timeout=180.)
    ic=_model(model,'ica',y)
    if p['reference_id']!=model['reference_id']:raise ValueError('参考配置不一致')
    _number(p['timeout'],'timeout',1,600)
    if not isinstance(p['octave'],str) or not p['octave'].strip():raise ValueError('octave 需要可执行文件路径')
    if y.info['sfreq']<100 or y.info['lowpass']<40:raise ValueError('MARA 频谱特征需要采样率≥100 Hz、输入低通≥40 Hz')
    duration=(y.n_times if isinstance(y,mne.io.BaseRaw) else len(y)*len(y.times))/y.info['sfreq']
    if duration<30:raise ValueError('MARA 需要至少 30 秒信号计算 15 秒局部偏度')
    eeg,xyz=_native_eeg(y,ic)
    native,provenance=_run_native('mara',eeg,p['source_root'],p['octave'],p['timeout'],_PINS)
    probabilities=np.asarray(native['posterior'],float).reshape(-1);features=np.asarray(native['features'],float)
    if probabilities.shape!=(ic.n_components_,) or features.shape!=(6,ic.n_components_) or not np.isfinite(probabilities).all() or not np.isfinite(features).all() or np.any((probabilities<0)|(probabilities>1)):
        raise ValueError('MARA 原生特征/概率退化')
    candidates=np.asarray(native['indices'],int).reshape(-1)-1
    if np.any((candidates<0)|(candidates>=ic.n_components_)):raise ValueError('MARA 候选索引越界')
    return _out(y,model,candidates=candidates,artifact_probabilities=probabilities,normalized_features=features,
        feature_names=['current_density_norm','pattern_range','local_skewness','lambda','power_8_13_Hz','fit_error'],
        component_id=_legacy_id(ic),provenance=provenance,welch_mode='MATLAB R12+; explicit 1-second rectangular window; no detrend',
        classifier='linear pooled unbiased covariance, uniform class priors',classifier_diagnostics=native['classifier'],coordinates_mm=xyz)
