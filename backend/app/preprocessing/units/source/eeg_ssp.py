...
def _eeg_ssp_ssp_apply(op, x, model=None, **p):
    y = _copy(x)
    before = y.get_data().copy()
    if isinstance(y, mne.BaseEpochs) and y.baseline is not None:
        raise ValueError('先伪迹校正，后基线；不自动重做基线')
    _args(p, 'projs')
    if not p['projs'] or y.info['projs']:
        raise ValueError('需非空已审核 projector；模板禁止叠加已有 projector')
    y.add_proj(p['projs']).apply_proj()
    return _out(y, model, removed=before - y.get_data())

def eeg_ssp(op, x, model=None, **p):
    if op == 'ssp_apply':
        if model is not None:
            raise ValueError('此操作不接受模型')
        return _eeg_ssp_ssp_apply(op, x, model=model, **p)
