...
def _prep(x, model, p, methods):
    ...
    for name in allowed:
        ...
        if name=='deviation':
            ...
            if x.info.get('lowpass') is not None and x.info['lowpass']<=50:raise ValueError('HF ratio not applicable after lowpass <= 50 Hz')
