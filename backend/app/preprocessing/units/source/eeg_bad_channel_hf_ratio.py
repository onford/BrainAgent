...
def eeg_bad_channel_hf_ratio(op,x,model=None,**p):
    if op != 'hf_ratio_detect':raise ValueError("invalid operation")
    p=_params(p,{'do_detrend': True, 'matlab_strict': True, 'random_state': 0, 'reject_by_annotation': None, 'HF_zscore_threshold': 5.0})
    methods={'hfnoise':{k:p.pop(k) for k in ['HF_zscore_threshold']}}
