...
def subject_numbers(quote):
    ...
    lists = rf'{members}(?:\s*(?:,|;|and|、|和)\s*{members})*'
...
def validate_claim(claim, entry):
    ...
    if claim.claim_type == 'exclusion':
        explicit = re.search(r'\b(exclud\w*|remov\w*|discard\w*|reject\w*|omit\w*)\b|排除|剔除|舍弃', span, re.I)
        negated = re.search(r'\b(?:no|not|never|without)\b.{0,60}\b(?:exclud\w*|remov\w*|discard\w*|reject\w*|omit\w*)\b|未.{0,8}(?:排除|剔除)', span, re.I)
