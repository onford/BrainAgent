...
The purpose is model training. The initial recommendation is frozen before numerical execution; all recommended shared recipes are executed in that fixed order, without subsequent model edits or recommendations. The final recipe is selected deterministically by measured development utility. Development scores do not establish independent generalization.
