...
const descriptions: Record<string, string> = {
  ...
  'survey/local-inspection.json': '按记录组织的本地观测、共享配置与检查覆盖',
  'survey/observation-v2/local-inspection.json': '历史运行补充检查的结构化本地观测，检查时间单独记录',
  'survey/observation-v2/local-events.tsv': '补充检查提取的逐事件时间表',
  'survey/observation-v2/source-inventory.tsv': '补充检查时的本地文件清单',
  'survey/observation-v2/directory-tree.txt': '补充检查时的本地目录结构',
  'survey/observation-v2/triggers.tsv': '适配器的任务标签映射',
  'survey/observation-v2/reports/data-information.html': '分组本地观测与原运行的外部资料核对结论',
  'survey/local-events.tsv': '逐事件原始标签、起始秒、持续秒和样点位置',
  ...
  'survey/reports/dataset-basic.html': '数据集名称、版本、许可、来源与本轮任务范围',
  'survey/reports/data-information.html': '本地文件、官网仓库和官方论文的逐项核对与证据',
  'survey/reports/statistics.html': '被试、记录、时长、采样率、通道与事件统计',
  'survey/reports/literature-usage.html': '使用该数据集的分析与算法文献，供后续方案参考',
  'survey/reports/literature-discussion.html': '讨论数据集特点、问题与排除事项的文献，供接入核对',
  'survey/reports/literature-preprocessing.html': '同类数据预处理文献与仓库，供方案设计参考',
  ...
  'collection/delta.tsv': '筛选前后的统计变化、原因与未知项目',
  'collection/audit.json': '十五类接入检查、处理规则和代码版本',
  'collection/literature-exclusions.json': '文献排除对象与本地编号的核对及处理',
  'collection/source-integrity.json': '所选原始文件的哈希与前后不变性核验',
  'collection/standardization.json': '标准化范围、事件保留和实际验证级别',
  'collection/channel-mapping.tsv': '原始通道名称、顺序与标准通道的对应',
  'collection/event-mapping.tsv': '全部原始事件与标准事件、训练选择的对应',
