...
<template>
  <div v-if="data" class="assessment">
    ...
    <section v-else-if="axis === 'quality'">
      ...
      <details class="reading-note"><summary>{{ t('Stage comparison and signal units') }}</summary><p class="note">{{ t('Lower amplitude or high-frequency power does not necessarily mean higher quality. Check reference, passband, and physical voltage units before comparing stages.') }}</p></details>
