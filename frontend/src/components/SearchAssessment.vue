...
const models: Record<string, string> = { csp_lda: 'CSP + LDA', fbcsp: 'FBCSP', ts_lr: '切空间 + 逻辑回归', fgmdm: 'FgMDM', ea_fbcsp: '逐频带 EA + FBCSP', logvar_lr: '对数方差 + 逻辑回归' }
...
<template>
  <div v-if="data" class="assessment">
    ...
    <section v-if="axis === 'utility'">
      ...
      <p class="note">三个主模型全部完成才产生选择分数。下四分位与离散程度描述被试差异；Brier 和对数损失越低越好。开发得分经过反复选择，尚未经独立确认。</p>
