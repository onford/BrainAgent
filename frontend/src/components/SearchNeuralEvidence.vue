...
<template>
  <section class="neural-evidence">
    ...
    <template v-else>
      <p>先验用于首次推荐和测量解释。筛查阈值不代表生理诊断标准，也不要求每个人出现典型 μ / β 活动。</p>
      ...
      <p v-else>已执行 {{ diagnostics.length }} 项诊断。测量仅供查看，不触发流程改写。</p>
      <article v-for="d in diagnostics" :key="d.id">
        ...
        <details v-for="r in d.prior_evaluation?.rules" :key="r.id">
          ...
          <p>解释提示：{{ r.implication }}</p><p>竞争解释：{{ r.alternative }}</p><p>保护条件：{{ r.protection }}</p>
