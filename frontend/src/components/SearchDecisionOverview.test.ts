...
describe('SearchDecisionOverview', () => {
  it('does not select the highest candidate locally and exposes initial recommendation failures', async () => {
    const state = { id: 'run', status: 'failed', protocol: { utility_version: 2 }, selected_candidate_id: null,
      ...
      actions: [{ index: 1, action: 'initial_recommendation', status: 'failed', candidate_id: 'c1', reason: '检查频带', result: { prediction_checks: { checks: [{ metric: 'x', explanation: '预期改善', status: 'contradicted', before: .2, after: .1 }] } } }],
    ...
    expect(wrapper.text()).not.toContain('预期改善')
