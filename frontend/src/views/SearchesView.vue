...
const strategies: Record<SearchStrategy, string> = { adaptive: '自适应', random: '随机', exhaustive: '穷举', one_shot: '一次性提案对照' }
      ...
      <section class="main-content" :aria-busy="loading">
        ...
        <template v-if="!id">
          ...
          <form class="card create-form" aria-label="创建预算搜索" @submit.prevent="start">
            ...
            <fieldset class="wide"><legend>搜索策略</legend><div class="strategy-options"><label v-for="(name, key) in strategies" :key="key" :class="{ chosen: strategy === key }"><input v-model="strategy" type="radio" name="strategy" :value="key" />{{ name }}</label></div><p v-if="strategy === 'one_shot'" class="strategy-hint muted">LLM 在开始时一次性提出候选顺序，后续按冻结顺序执行，不根据中途评价调整提案。</p></fieldset>
