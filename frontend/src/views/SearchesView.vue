...
const workflowId = ref(''), strategy = ref<SearchStrategy>('one_shot'), seed = ref(42)
const trainSubjects = ref(''), developmentSubjects = ref('')
const formBudget = reactive({ max_candidates: 48, max_seconds: 86400, max_memory_mb: '' as number | string, max_disk_mb: '' as number | string })
...
const phases: Record<string, string> = { get freeze_panel() { return t('Freeze development panel') }, get candidate() { return t('Evaluate candidate') }, get recommendation() { return t('Initial recommendation') }, get finished() { return t('Search ended') } }
const actionLabels: Record<string, string> = { get initial_recommendation() { return t('Initial recommendation') } }
...
const strategies: Record<SearchStrategy, string> = { get one_shot() { return t('Initial recommendation') } }
...
const tabs = { get overview() { return t('Decision overview') }, get candidates() { return t('Candidate comparison') }, get methods() { return t('Search space') }, get assessment() { return t('Multidimensional evaluation') }, get rounds() { return t('Frozen recommendation and execution records') }, get subjects() { return t('Development subjects') }, get artifacts() { return t('Reports / files') } }
const budgetFields = [
...
const supportedSearch = computed(() => current.value?.schema_version === '1' && ['2', '3', '4'].includes(current.value.protocol?.version ?? ''))
const currentProtocol = computed(() => supportedSearch.value && current.value?.protocol?.version === '4')
const active = computed(() => currentProtocol.value && current.value && ['preparing', 'running'].includes(current.value.status))
...
const canRetry = computed(() => currentProtocol.value && current.value && ['failed', 'interrupted', 'cancelled'].includes(current.value.status))
const candidates = computed(() => current.value?.candidates ?? [])
const actions = computed(() => current.value?.actions ?? [])
...
const multiMetric = computed(() => ['3', '4'].includes(String(current.value?.protocol?.version)) || !!current.value?.protocol?.assessment)
...
const displayedPhase = computed(() => {
  if (active.value && current.value?.request.strategy === 'one_shot' && current.value.actions?.some(action => action.action === 'initial_recommendation' && action.status === 'running')) return t('Initial recommendation')
...
function request(): SearchRequest {
  ...
  const selectedBudget: SearchBudget = {
    max_candidates: Number(formBudget.max_candidates),
    max_seconds: Number(formBudget.max_seconds),
  ...
  <main class="search-page">
    ...
    <div class="search-layout">
      ...
      <section class="main-content" :aria-busy="loading">
        ...
        <template v-if="!id">
          <header class="page-heading"><p class="eyebrow">OFFLINE SEARCH</p><h1>{{ t('Initial recommendation and fixed evaluation') }}</h1><p class="muted">{{ t('Recommend once before evaluation, then execute the frozen recipes in order.') }}</p></header>
          ...
          <form class="card create-form" :aria-label="t('Create a budgeted search')" @submit.prevent="start">
            ...
            <p class="wide muted" :aria-label="t('Default evaluation')">{{ t('Subject-grouped cross-validation uses up to five folds, with each subject held out once. The primary score averages subject-mean BA over EEGNet seeds 17, 42, and 2026; all three must complete. CSP-LDA is a benchmark only. The 25 signal-quality and 14 reconstruction metrics are reported separately.') }}</p><fieldset class="wide"><legend>{{ t('Search strategy') }}</legend><div class="strategy-options"><label v-for="(name, key) in strategies" :key="key" :class="{ chosen: strategy === key }"><input v-model="strategy" type="radio" name="strategy" :value="key" />{{ name }}</label></div><p v-if="strategy === 'one_shot'" class="strategy-hint muted">{{ t('The LLM proposes the candidate order once at the start. Execution follows that fixed order without adapting proposals to interim evaluations.') }}</p></fieldset>
        ...
        <template v-else-if="current">
          <header class="page-heading run-heading"><div><p class="eyebrow">OFFLINE SEARCH · {{ current.id }}</p><h1>{{ supportedSearch ? t('Initial recommendation and fixed evaluation') : t('Budgeted search record') }} <span class="badge" :class="current.status">{{ label(current.status) }}</span></h1><p class="muted"><RouterLink :to="{path:'/workflows',query:{id:current.workflow_id}}">{{ t('Source workflow {0} ↗', { 0: current.workflow_id }) }}</RouterLink>{{ t('· {0} · Seed {1} · Updated {2}', { 0: strategies[current.request?.strategy] ?? current.request?.strategy ?? '—', 1: current.request?.seed ?? '—', 2: date(current.updated_at) }) }}</p></div><div class="run-controls"><span v-if="!currentProtocol" class="muted">{{ t('Read-only record') }}</span><button v-if="active" :disabled="busy || current.cancellation_requested" @click="control('cancel')">{{ current.cancellation_requested ? t('Stopping search…') : busy ? t('Processing…') : t('Stop search') }}</button><button v-if="canRetry" class="primary" :disabled="busy" @click="control('retry')">{{ busy ? t('Processing…') : t('Retry search') }}</button></div></header>
          <p class="notice">{{ t('Development evaluation guides selection. It is not an independent test or a direct measure of neural signal quality.') }}</p>
          ...
          <details class="card evaluator-summary" :aria-label="t('Scoring definitions')">
            <summary>{{ t('Scoring') }}<span class="muted">· {{ multiMetric ? t('{0}, signal quality, and reconstruction experiments', { 0: utilityLabel }) : evaluatorLabel }}</span></summary><p v-if="!cspEvaluation">{{ t('Evaluator: {0}. Scores and selections follow the saved records.', { 0: evaluatorLabel }) }}</p>
