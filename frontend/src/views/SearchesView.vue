...
const supportedSearch = computed(() => current.value?.schema_version === '1' && current.value.protocol?.version === '2')
  ...
  <main class="search-page">
    ...
    <div class="search-layout">
      ...
      <section class="main-content" :aria-busy="loading">
        ...
        <template v-if="!id">
          <header class="page-heading"><p class="eyebrow">OFFLINE SEARCH</p><h1>诊断预算策略搜索</h1><p class="muted">设定有限预算，根据信号诊断比较预处理策略，保留假设、预测核验与评估记录。</p></header>
          ...
          <form class="card create-form" aria-label="创建预算搜索" @submit.prevent="start">
            ...
            <p class="wide muted" aria-label="默认评估方式">默认按被试分组交叉验证，最多 5 折；全部被试各作为开发被试一次。主评分使用 CSP + 收缩 LDA 的被试平均 BA。</p><fieldset class="wide"><legend>搜索策略</legend><div class="strategy-options"><label v-for="(name, key) in strategies" :key="key" :class="{ chosen: strategy === key }"><input v-model="strategy" type="radio" name="strategy" :value="key" />{{ name }}</label></div><p v-if="strategy === 'random'" class="strategy-hint muted">随机安排候选执行顺序，最终仍按开发主评分选择。</p><p v-if="strategy === 'one_shot'" class="strategy-hint muted">LLM 在开始时一次性提出候选顺序，后续按冻结顺序执行，不根据中途评价调整提案。</p></fieldset>
        ...
        <template v-else-if="current">
          ...
          <section class="card budget-panel" aria-label="进度与预算">
            ...
            <div v-else-if="panel" class="panel-summary" aria-label="评估面板">
              ...
              <div v-if="panel.folds?.length" class="table-scroll"><table aria-label="评估折次"><thead><tr><th>折次</th><th>训练被试</th><th>开发被试</th></tr></thead><tbody><tr v-for="fold in panel.folds" :key="fold.id"><td>{{ fold.id }}</td><td>{{ fold.train_subjects.join('、') }}</td><td>{{ fold.development_subjects.join('、') }}</td></tr></tbody></table></div>
          ...
          <section class="card evaluator-summary" aria-label="评分与适配含义">
            <h2>评分与适配</h2><p v-if="!cspEvaluation">评价器：{{ evaluatorLabel }}。分数与选择结果按保存的记录展示。</p>
            <p v-if="cspEvaluation">主评分为 CSP + 收缩 LDA 的被试平均 BA：先计算每名开发被试的左右手平衡准确率，再对被试等权平均。CSP 与分类器在每折训练被试上拟合。</p>
            <p v-if="cspEvaluation">次要对照使用 log-variance（对数方差）+ Logistic Regression（逻辑回归），用于检查表示与预测行为。</p>
