...
const multiMetric = computed(() => current.value?.protocol?.version === '3')
      ...
      <section class="main-content" :aria-busy="loading">
        ...
        <template v-if="!id">
          ...
          <form class="card create-form" aria-label="创建预算搜索" @submit.prevent="start">
            ...
            <p class="wide muted" aria-label="默认评估方式">默认按被试分组交叉验证，最多 5 折；全部被试各作为开发被试一次。主评分为 CSP-LDA、FBCSP、TS-LR 三种模型的被试平均 BA 等权均值；信号质量与重建指标分别展示。</p><fieldset class="wide"><legend>搜索策略</legend><div class="strategy-options"><label v-for="(name, key) in strategies" :key="key" :class="{ chosen: strategy === key }"><input v-model="strategy" type="radio" name="strategy" :value="key" />{{ name }}</label></div><p v-if="strategy === 'random'" class="strategy-hint muted">随机安排候选执行顺序，最终仍按开发主评分选择。</p><p v-if="strategy === 'one_shot'" class="strategy-hint muted">LLM 在开始时一次性提出候选顺序，后续按冻结顺序执行，不根据中途评价调整提案。</p></fieldset>
        ...
        <template v-else-if="current">
          ...
          <details class="card evaluator-summary" aria-label="评分与适配含义">
            <summary>评分与适配 <span class="muted">· {{ multiMetric ? '三模型训练效用、信号质量、重建实验' : evaluatorLabel }}</span></summary><p v-if="!cspEvaluation">评价器：{{ evaluatorLabel }}。分数与选择结果按保存的记录展示。</p>
            <p v-if="multiMetric">主评分取 CSP-LDA、FBCSP、TS-LR 三个被试宏平均 BA 的等权均值，三种模型须完整。质量和重建实验独立呈现；CSP 明细作为共同锚点保留。</p><p v-else-if="cspEvaluation">主评分为 CSP + 收缩 LDA 的被试平均 BA：先计算每名开发被试的左右手平衡准确率，再对被试等权平均。CSP 与分类器在每折训练被试上拟合。</p>
