      ...
      <section class="results">
        ...
        <template v-if="current">
          <section class="panel progress-panel"><div class="section-heading"><h2>{{sourceSummary?.profile.name ?? 'EEG 数据流程'}}</h2><span class="badge">{{labels[current.status]}}</span></div><p role="status">{{completedCount}} / 6 个模块已完成</p><progress :value="completedCount" max="6" aria-label="流程进度" />
            <ol class="stages"><li v-for="(stage,index) in current.stages" :key="stage.name" :class="stage.status"><span class="stage-number">{{stage.status==='completed'?'✓':index+1}}</span><div><strong>{{stage.label}}</strong><small>{{labels[stage.status]}}</small></div></li></ol>
            <template v-for="stage in current.stages" :key="stage.name"><details v-if="stage.error" class="stage-error"><summary>{{stage.label}}：查看错误详情</summary><p class="error">{{stage.error}}</p></details></template>
...
<style scoped>
.stage-error{margin:12px 0;border:1px solid #ecd7d1;border-radius:8px;padding:10px 14px}.stage-error summary{cursor:pointer;color:#9f3828}.stage-error .error{max-height:260px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere}
